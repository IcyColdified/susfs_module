...
#define PRT_MSG_IF_OPERATION_NOT_SUPPORTED(x, cmd)                             \
  if (x == -1)                                                                 \
  log("[-] CMD: '0x%x', SUSFS operation not supported, please enable it in "   \
      "kernel\n",                                                              \
      cmd)
...
static void print_help(void) {
  ...
  log("         |--> Added path and all its sub-paths will be hidden from "
      "several syscalls\n");
  log("         |--> Please be reminded that the target path must be added "
      "after the bind mount or overlay operation, otherwise it won't be "
      "effective\n");
  log("\n");
  log("        add_sus_mount <mounted_path>\n");
  log("         |--> Added mounted path will be hidden from "
      "/proc/self/[mounts|mountinfo|mountstats]\n");
  log("         |--> Please be reminded that the target path must be added "
      "after the bind mount or overlay operation, otherwise it won't be "
      "effective\n");
  log("\n");
  log("        add_sus_kstat_statically </path/of/file_or_directory> <ino> "
      "<dev> <nlink> <size>\\\n");
  log("                                 <atime> <atime_nsec> <mtime> "
      "<mtime_nsec> <ctime> <ctime_nsec>\n");
  log("                                 <blocks> <blksize>\n");
  log("         |--> Use 'stat' tool to find the format:\n");
  log("                  ino -> %%i, dev -> %%d, nlink -> %%h, atime -> %%X, "
      "mtime -> %%Y, ctime -> %%Z\n");
  log("                  size -> %%s, blocks -> %%b, blksize -> %%B\n");
  log("         |--> e.g., %s add_sus_kstat_statically '/system/addon.d' "
      "'1234' '1234' '2' '223344'\\\n",
      TAG);
  log("                       '1712592355' '0' '1712592355' '0' '1712592355' "
      "'0' '1712592355' '0'\\\n");
  log("                       '16' '512'\n");
  ...
  log("         |--> e.g., %s add_sus_kstat_statically '/system/addon.d' "
      ...
      TAG);
  log("                       '1712592355' 'default' '1712592355' 'default' "
      "'1712592355' 'default'\\\n");
  log("                       'default' 'default'\n");
  log("\n");
  log("        add_sus_kstat </path/of/file_or_directory>\n");
  log("         |--> Add the desired path BEFORE it gets bind mounted or "
      "overlayed, this is used for storing original stat info in kernel "
      "memory\n");
  log("         |--> This command must be completed with <update_sus_kstat> "
      "later after the added path is bind mounted or overlayed\n");
  log("\n");
  log("        update_sus_kstat </path/of/file_or_directory>\n");
  log("         |--> Add the desired path you have added before via "
      "<add_sus_kstat> to complete the kstat spoofing procedure\n");
  log("         |--> This updates the target ino, but size and blocks are "
      "remained the same as current stat\n");
  log("\n");
  log("        update_sus_kstat_full_clone </path/of/file_or_directory>\n");
  log("         |--> Add the desired path you have added before via "
      "<add_sus_kstat> to complete the kstat spoofing procedure\n");
  log("         |--> This updates the target ino only, other stat members are "
      "remained the same as the original stat\n");
  log("\n");
  log("        add_try_umount </path/of/file_or_directory> <mode>\n");
  log("         |--> Added path will be umounted from KSU for all UIDs that "
      "are NOT su allowed, and profile template configured with umount\n");
  log("         |--> <mode>: 0 -> umount with no flags, 1 -> umount with "
      "MNT_DETACH\n");
  log("         |--> NOTE: susfs umount takes precedence of ksu umount\n");
  log("\n");
  log("        set_uname <release> <version>\n");
  log("         |--> NOTE: Only 'release' and <version> are spoofed as others "
      "are no longer needed\n");
  log("         |--> Spoof uname for all processes, set string to 'default' to "
      "imply the function to use original string\n");
  log("         |--> e.g., set_uname '4.9.337-g3291538446b7' 'default'\n");
  log("\n");
  log("        enable_log <0|1>\n");
  log("         |--> 0: disable susfs log in kernel, 1: enable susfs log in "
      "kernel\n");
  log("\n");
  log("        sus_su <0|1>\n");
  log("         |--> NOTE-1:\n");
  log("                This feature allows user to disable kprobe hooks made "
      "by ksu, and instead,\n");
  log("                a sus_su character device driver with random name will "
      "be created, and user\n");
  log("                need to use a tool named 'sus_su' together with a path "
      "file in same current directory\n");
  log("                named '" SUS_SU_CONF_FILE_PATH
      "' to get a root shell from the sus_su driver'\n");
  log("         |--> NOTE-2:\n");
  log("                To use it please see the service.sh from module "
      "template\n");
  log("         |--> 0: enable core ksu kprobe hooks and disable sus_su "
      "driver\n");
  log("         |--> 1: disable the core ksu kprobe hooks and enable sus_su "
      "driver\n");
  log("\n");
}
...
int main(int argc, char *argv[]) {
  ...
  if (argc == 3 && !strcmp(argv[1], "add_sus_path")) {
    ...
    struct stat sb;
    char *endptr;
    unsigned long ino, dev, nlink, size, atime, atime_nsec, mtime, mtime_nsec,
        ctime, ctime_nsec, blksize;
    long blocks;
    ...
    prctl(KERNEL_SU_OPTION, CMD_SUSFS_ADD_SUS_KSTAT_STATICALLY, &info, NULL,
          &error);
    ...
    info.spoofed_blocks =
        sb.st_blocks; // use the current blocks, not the spoofed one
    ...
    if (!strcmp(p_abs_path, "/system") || !strcmp(p_abs_path, "/vendor") ||
        !strcmp(p_abs_path, "/product") ||
        !strcmp(p_abs_path, "/data/adb/modules") ||
        !strcmp(p_abs_path, "/debug_ramdisk") || !strcmp(p_abs_path, "/sbin")) {
      log("[-] %s cannot be added to try_umount, because it will be umounted "
          "by ksu lastly\n",
          p_abs_path);
      return 1;
    }
    ...
    if (!strcmp(argv[2], "1")) {
      ...
      if (system("export DRV_PATH=`cat " SUS_SU_CONF_FILE_PATH
                 "`; chmod 666 ${DRV_PATH} && chcon u:object_r:null_device:s0 "
                 "${DRV_PATH}")) {
