...
static const char *sus_su_token =
    "!@#$SU_IS_SUS$#@!-"
    "pRE6W9BKXrJr1hEKyvDq0CvWziVKbatT8yzq06fhtrEGky2tVS7Q2QTjhtMfVMGV";
...
static char **get_new_envp(void) {
  ...
  // Allocate space for new environment (existing + custom PATH + NULL
  // terminator)
  ...
  size_t new_path_env_size = strlen(orig_path_env) + strlen(additional_paths) +
                             7; /* 5 for 'PATH=', 1 for ':', 1 for '\0' */
  ...
  snprintf(new_path_env_buf, new_path_env_size, "PATH=%s:%s", orig_path_env,
           additional_paths);
